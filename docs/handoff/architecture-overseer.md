...
| **🚨 deepcopy slot_context 회귀** (2026-05-08, push 3cfa26a) | agent.py:738 `copy.deepcopy(slot_context)` 도입 후 trigger pipeline silent fail. backend redis pub 성공하지만 [TRIGGER] / [TIMING] 로그 0건. asyncio task가 deepcopy에서 죽음 또는 spawn 안 됨. | Codex P1 fix 의도(nested mutable 격리)는 정합하나 slot_context 안 nested 중 deepcopy 안 되는 객체 가능성. 정확한 원인 미확정 — `_log_detached_task_result`가 exception swallow 또는 deepcopy 자체 RecursionError. **lesson**: 외부 영향 큰 객체(`copy.deepcopy`)를 핵심 hot path에 도입할 때 라이브 검증 필수. Codex 정적 분석으로는 silent fail 못 잡음. | hotfix: selective deepcopy — `dict()` shallow + `pre_extracted_signals` / `place_coord` / `manual_chosen_time`만 명시 deepcopy. P1 의도 유지 + 회귀 차단. 진짜 원인은 시연 후 별도 진단. |
