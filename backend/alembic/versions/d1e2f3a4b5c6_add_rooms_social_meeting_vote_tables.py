...
def upgrade() -> None:
    ...
    if not inspector.has_table("rooms"):
        ...
        # rooms가 이미 있으면 category 컬럼만 추가
        room_cols = [c["name"] for c in inspector.get_columns("rooms")]
        if "category" not in room_cols:
            op.add_column("rooms", sa.Column("category", sa.String(length=64), nullable=True))
    ...
    # --------------------------------------------------------------- vote_options
    if not inspector.has_table("vote_options"):
        op.create_table(
            "vote_options",
            sa.Column("id", sa.Integer(), nullable=False),
    ...
    # ------------------------------------------------------------- vote_responses
    if not inspector.has_table("vote_responses"):
        op.create_table(
            "vote_responses",
            sa.Column("id", sa.Integer(), nullable=False),
            sa.Column("vote_id", sa.Integer(), nullable=False),
    ...
    # --------------------------------------------------------------- ai_memories
    if not inspector.has_table("ai_memories"):
        op.create_table(
            "ai_memories",
            sa.Column("id", sa.Integer(), nullable=False),
            sa.Column("user_id", sa.Integer(), nullable=False),
