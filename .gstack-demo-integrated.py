...
# 시나리오: 이번주 평일 모두 거부 → 다음주 자동확장.
# 절대 날짜("5월 13일") 표기는 시연일이 바뀌면 stale → entity_extraction LLM 경로로
# 빠지면서 slot.py의 has_date 체크가 거부 컨텍스트를 무시하고 partial_info_acknowledged
# 종료. 상대 요일("이번 주 수")로 표기하면 pre-extraction 정규식이 REJECTED_DATES로
# 라벨링해 vote_card 노드의 다음주 자동확장 분기를 탐.
# TIMEBAR_DATE는 fallback 값 — ACT 3 진입 직전에 추천 카드 텍스트에서 동적 파싱.
TIMEBAR_DATE = "2026-05-25"
TIMEBAR_SLOT_START = 18
TIMEBAR_SLOT_END = 19

# ACT 2 거부 메시지 — 상대 요일 표기로 pre-extraction 경로 보장.
# 호스트 GCal busy 패턴에 따라 backend가 평일 추천 가능한 첫 날짜를 잡음.
ACT2_MESSAGES = [
    ("host", "다들 시험 끝나고 한번 보자!"),
    ("수현", "이번 주 수요일은 동아리 MT라 안 돼"),
    ("민수", "목요일은 본가 내려가야 해서 패스"),
    ("예린", "금요일은 시험 보강 잡혀서 안 될 것 같아… 다음주 어때?"),
]
