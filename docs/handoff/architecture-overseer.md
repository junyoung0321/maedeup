...
| **🚨 deepcopy slot_context 회귀 (수정 — 진단 진행 중)** (2026-05-08, push 3cfa26a) | agent.py:738 `copy.deepcopy(slot_context)` 도입 후 trigger pipeline silent fail. selective deepcopy도 fail. 단순 `dict()` revert로 hotfix 적용했으나 여전히 fail 보고. | **deepcopy가 진짜 원인 아닐 가능성 강함**. 가장 강한 가설은 **`schedule_auto_trigger_fired:{room_pk}:{snapshot_hash}` NX lock 5분 잔존** — `social.py:113-145 publish_schedule_auto_trigger`이 NX 잡고 첫 fail 후에도 5분 동안 같은 snapshot_hash trigger 모두 skip. 두 번째 가설은 `nx_autotrigger:{room_id}` 60s 잔존. **lesson**: silent fail 추적 시 NX/lock state를 코드 회귀보다 먼저 점검. Redis state가 코드 hotfix 효과를 가릴 수 있음. | hotfix 1: 단순 dict() revert (적용됨). 진단 1: `redis-cli KEYS` 로 NX 점검 + `FLUSHDB` 후 재현. `[AUTO_TRIGGER]` 진단 logging 추가 박힘 — 어느 단계에서 끊기는지 정확한 위치 잡기. |
